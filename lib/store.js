...
async function createBugTaskAndNotify(api, reporterName, assigneeId, reporterId) {
  let taskUrl = null;
  try {
    const notion = require('./notion');
    const task = await notion.createBugTask(api, { reporterName, assigneeId, reporterId });
...
async function createIncident(body = {}, opts = {}) {
  ...
  const initHistory = [{ status: 'Open', at: now }];
  ...
  if (!configured()) {
    const api = toApi(row);
    api.statusHistory = initHistory;
    api.reporterName = reporterName;
    mem.push(api);
    ...
    await createBugTaskAndNotify(api, reporterName, assigneeId, reporterId);
    return api;
  }
  const [inserted] = await pg('POST', '', row, { Prefer: 'return=representation' });
  const api = toApi(inserted);
  api.reporterName = reporterName; // shown in Slack for external (non-Slack) reporters
  ...
  api.statusHistory = initHistory;
  // Persist reporter/assignee/status-history separately so a later status change
  // can re-render the head + compute time-in-status. Best-effort and isolated:
  // if the columns don't exist yet, this fails silently WITHOUT losing the
  // Slack-thread patch below (which would break threading).
  try { await pg('PATCH', `?id=eq.${encodeURIComponent(api.id)}`, { reporter_id: api.reporterId, assignee_slack_id: api.assigneeSlackId, status_history: initHistory }); } catch (_) {}
  // Persist the reporter's display name separately (isolated) so a missing
  // reporter_name column can't break the patch above. Only external reporters
  // need it — internal reporters are shown via their Slack id.
  if (reporterName) { try { await pg('PATCH', `?id=eq.${encodeURIComponent(api.id)}`, { reporter_name: reporterName }); } catch (_) {} }
...
async function updateIncident(id, patch = {}) {
  ...
  if (!configured()) {
    ...
    if (changes.status != null && changes.status !== prevStatus) {
      inc.statusHistory = [...(inc.statusHistory || []), { status: changes.status, at: changes.updated_at }];
      try {
        await slackLib.postStatusChange(inc, changes.status);
        await slackLib.setStatusReaction(inc, changes.status);
      } catch (_) {}
    }
  ...
  if (changes.status != null && existing && changes.status !== existing.status && existing.slack_ts) {
    try {
      ...
      await slackLib.setStatusReaction(existing, changes.status);
    } catch (_) {}
  }
  // Append to the status-change history (best-effort; tolerates a missing column).
  if (changes.status != null && existing && changes.status !== existing.status) {
    try {
      const hist = [...(Array.isArray(updated.status_history) ? updated.status_history : []), { status: changes.status, at: changes.updated_at }];
      const [h] = await pg('PATCH', `?id=eq.${encodeURIComponent(id)}`, { status_history: hist }, { Prefer: 'return=representation' });
      if (h) updated.status_history = h.status_history;
    } catch (_) {}
  }
