...
async function createBugTaskAndNotify(api, reporterName) {
  let taskUrl = null;
  try {
    const notion = require('./notion');
    const task = await notion.createBugTask(api, { reporterName });
...
async function createIncident(body = {}, opts = {}) {
  ...
  if (!configured()) {
    const api = toApi(row);
    mem.push(api);
    try { const s = await slackLib.postIncidentCreated(api, reporterId); if (s) { api.slackTs = s.ts; api.slackChannel = s.channel; api.slackUrl = s.url || null; } } catch (_) {}
    await createBugTaskAndNotify(api, reporterName);
    return api;
  }
  ...
  try {
    const s = await slackLib.postIncidentCreated(api, reporterId);
