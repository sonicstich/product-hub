...
async function createIncident(body = {}) {
  ...
  if (!configured()) {
    const api = toApi(row);
    mem.push(api);
    try { const s = await slackLib.postIncidentCreated(api); if (s) { api.slackTs = s.ts; api.slackChannel = s.channel; } } catch (_) {}
    return api;
  }
  ...
  try {
    ...
    if (s) {
      const [upd] = await pg('PATCH', `?id=eq.${encodeURIComponent(api.id)}`,
        { slack_ts: s.ts, slack_channel: s.channel }, { Prefer: 'return=representation' });
...
async function updateIncident(id, patch = {}) {
  ...
  if (!configured()) {
    ...
    if (changes.status != null && changes.status !== prevStatus) {
      try { await slackLib.postStatusChange(inc, changes.status); } catch (_) {}
    }
    return inc;
  }
  ...
  if (changes.status != null && existing && changes.status !== existing.status && existing.slack_ts) {
    try { await slackLib.postStatusChange(existing, changes.status); } catch (_) {}
  }
