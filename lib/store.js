...
async function createIncident(body = {}) {
  ...
  if (!configured()) {
    const api = toApi(row);
    mem.push(api);
    try { const s = await slackLib.postIncidentCreated(api); if (s) { api.slackTs = s.ts; api.slackChannel = s.channel; api.slackUrl = s.url || null; } } catch (_) {}
    return api;
  }
  ...
  try {
    const s = await slackLib.postIncidentCreated(api);
    if (s) {
      const [upd] = await pg('PATCH', `?id=eq.${encodeURIComponent(api.id)}`,
        { slack_ts: s.ts, slack_channel: s.channel, slack_url: s.url || null }, { Prefer: 'return=representation' });
...
function toReleaseApi(row) {
  return { cycleKey: row.cycle_key, num: row.cycle_num, ios: row.ios || {}, android: row.android || {} };
}
...
async function saveRelease(body = {}) {
  ...
  const row = {
    ...
    ios: sanitizeRel(body.ios),
    android: sanitizeRel(body.android),
    updated_at: new Date().toISOString(),
  };
