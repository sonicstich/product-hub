...
// Fixed co-assignee always tagged on incidents (the PM). Slack member id;
// override with INCIDENT_SELF_SLACK_ID.
const SELF_SLACK_ID = () => process.env.INCIDENT_SELF_SLACK_ID || 'U09NZ2BDSAY'; // Emil Mubarakov
...
function incidentBlocks(inc) {
  ...
  // Assignee = the picked dev (or the area map) PLUS a fixed co-assignee (the
  // PM), always — EXCEPT when "Nobody" ('none') is chosen, which tags no one.
  let owners = [];
  if (assigneeSlackId !== 'none') {
    const picked = (assigneeSlackId ? [assigneeSlackId] : assigneesFor(inc.areas)).filter(Boolean);
    owners = [...new Set([...picked, ...(SELF_SLACK_ID() ? [SELF_SLACK_ID()] : [])])];
  }
...
async function postIncidentCreated(inc, reporterId, assigneeSlackId) {
  ...
  // New incidents start unresolved → mark them in-progress with the spinner.
  try { await setStatusReaction({ slackTs: data.ts, slackChannel: data.channel }, inc.status || 'Open'); } catch (_) {}
  // Post the screenshots into the thread (best-effort; needs files:write).
  try { await postThreadImages(data.channel, data.ts, inc.images); } catch (_) {}
  let url = null;
...
// Reflect the incident's state with a reaction on the head message: a spinner
// (:loading:) while it's unresolved so people see it's in progress, and a ✅
// once Resolved. Needs the reactions:write scope. Each reaction call is
// isolated and best-effort, so one failing (already-reacted, missing reaction,
// or the custom :loading: emoji not installed) doesn't block the other.
const PROGRESS_EMOJI = () => process.env.INCIDENT_PROGRESS_EMOJI || 'loading';
async function setStatusReaction(inc, status) {
  const ts = inc && (inc.slack_ts || inc.slackTs);
  const ch = (inc && (inc.slack_channel || inc.slackChannel)) || CHANNEL();
  if (!slackConfigured() || !ts) return null;
  const resolved = status === 'Resolved';
  const add    = resolved ? 'white_check_mark' : PROGRESS_EMOJI();
  const remove = resolved ? PROGRESS_EMOJI() : 'white_check_mark';
  try { await call('reactions.remove', { channel: ch, timestamp: ts, name: remove }); } catch (_) {}
  try { await call('reactions.add',    { channel: ch, timestamp: ts, name: add }); } catch (_) {}
  return true;
}

// DM a user directly. Opens (or reuses) the bot↔user IM channel, then posts.
// Needs the `im:write` scope on the bot (and the user must allow app DMs).
// Best-effort; returns null on any failure.
async function dmUser(userId, text, blocks) {
  if (!slackConfigured() || !userId) return null;
  const open = await call('conversations.open', { users: userId });
  const channel = open && open.channel && open.channel.id;
  if (!channel) return null;
  await call('chat.postMessage', { channel, text, blocks, unfurl_links: false });
  return true;
}

// DM the task's creator a link to the Notion task they just filed, so they can
// follow its status. `title` is the task title, `taskUrl` the Notion page.
async function dmTaskCreated(userId, taskUrl, title) {
  if (!slackConfigured() || !userId || !taskUrl) return null;
  const t = (title || 'your task').slice(0, 200);
  const body = 'You can follow its current status right inside the task in Notion. '
    + 'The related dev task will be linked inside it too, so you can track the progress there :pepe:\n'
    + 'If anything needs clarifying, the team will reach out to you.';
  const blocks = [
    { type: 'section', text: { type: 'mrkdwn', text: `:on-it: Your task has been created:\n*<${taskUrl}|${t}>*` } },
    { type: 'section', text: { type: 'mrkdwn', text: body } },
  ];
  return dmUser(userId, `Task created: ${t}`, blocks);
}

// Upload incident screenshots (data: URLs) into the incident's Slack thread via
// the external-upload flow (getUploadURLExternal → PUT bytes → completeUpload).
// Needs the files:write scope. Best-effort — never throws to the caller.
async function postThreadImages(channel, thread_ts, images) {
  if (!slackConfigured() || !channel || !thread_ts || !Array.isArray(images) || !images.length) return null;
  const token = TOKEN();
  const files = [];
  for (let i = 0; i < images.length && i < 3; i++) {
    const m = /^data:([^;]+);base64,(.+)$/.exec(images[i] || '');
    if (!m) continue;
    const bytes = Buffer.from(m[2], 'base64');
    const ext = ((m[1].split('/')[1]) || 'png').replace('jpeg', 'jpg');
    const filename = `screenshot-${i + 1}.${ext}`;
    const g = await fetch(`${SLACK_API}/files.getUploadURLExternal`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ filename, length: String(bytes.length) }),
    }).then(r => r.json()).catch(() => null);
    if (!g || !g.ok || !g.upload_url || !g.file_id) continue;
    const up = await fetch(g.upload_url, { method: 'POST', body: bytes }).catch(() => null);
    if (!up || !up.ok) continue;
    files.push({ id: g.file_id, title: filename });
  }
  if (!files.length) return null;
  await call('files.completeUploadExternal', { files, channel_id: channel, thread_ts });
  return true;
}

// TEMP diagnostic: post a test message + upload one image into its thread,
// surfacing the exact failure step. Remove once image upload is verified.
async function diagImages(dataUrl) {
  if (!slackConfigured()) return { error: 'slack not configured' };
  const token = TOKEN();
  const msg = await call('chat.postMessage', { channel: CHANNEL(), text: '[diagnostic] image-upload test' });
  const m = /^data:([^;]+);base64,(.+)$/.exec(dataUrl); const bytes = Buffer.from(m[2], 'base64');
  const g = await fetch(`${SLACK_API}/files.getUploadURLExternal`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ filename: 'diag.png', length: String(bytes.length) }) }).then(r => r.json());
  if (!g.ok) return { step: 'getUploadURLExternal', error: g.error };
  const up = await fetch(g.upload_url, { method: 'POST', body: bytes });
  if (!up.ok) return { step: 'putBytes', status: up.status };
  const c = await fetch(`${SLACK_API}/files.completeUploadExternal`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ files: [{ id: g.file_id, title: 'diag.png' }], channel_id: msg.channel, thread_ts: msg.ts }) }).then(r => r.json());
  return { ok: c.ok, error: c.error, step: 'completeUploadExternal' };
}

module.exports = { slackConfigured, postIncidentCreated, updateIncidentHead, postStatusChange, setStatusReaction, postTaskCreated, dmTaskCreated, diagImages };
