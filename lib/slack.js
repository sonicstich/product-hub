...
function incidentBlocks(inc) {
  ...
  // External reporters (email/password contractors) have no Slack account to
  // @mention — fall back to showing their name as plain text.
  const reporterName = inc.reporterName || inc.reporter_name || null;
  ...
  else if (reporterName) peopleLines.push(`*Reported by:* ${reporterName} _(external reporter)_`);
...
module.exports = { slackConfigured, postIncidentCreated, updateIncidentHead, postStatusChange, setStatusReaction, postTaskCreated, dmTaskCreated };
