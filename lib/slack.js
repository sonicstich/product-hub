...
function assigneesFor(areas) {
  ...
  if (!arr.length) { if (map.default) ids.add(map.default); return [...ids]; }
  arr.forEach(a => { const id = map[a] || map.default; if (id) ids.add(id); });
  return [...ids];
}
...
async function postIncidentCreated(inc, reporterId) {
  ...
  const owners = assigneesFor(inc.areas);
  const peopleLines = [];
  ...
  const pingIds = [...new Set([...(reporterId ? [reporterId] : []), ...owners])];
