...
async function createBugTask(inc, opts = {}) {
  ...
  // Assignee: an explicit pick from the form wins; otherwise fall back to the
  // area→assignee map (INCIDENT_ASSIGNEES_NOTION). Guard the id shape so a bad
  // value can't fail the whole create.
  const isUserId = v => typeof v === 'string' && /^[0-9a-fA-F-]{32,36}$/.test(v);
  const people = isUserId(opts.assigneeId) ? [opts.assigneeId] : notionAssigneesFor(inc.areas);
