...
// The product "Create Task" flow writes to the "Discovery Sprints" board — a
// data-source-model DB (so data_source_id parent + 2025-09-03 API). Its columns
// are Name/Status/Sprint/Assign/Priority/Type/🐋 Related Feature, so the form's
// richer fields (why, metric, description, mockup…) go into the page body.
const TASK_BOARD_DS_ID = () => process.env.TASK_BOARD_DS_ID || '3792e2b0-344d-4caa-9f05-67d3f88aba92';
...
async function createTask(body, opts = {}) {
  ...
  const props = {
    Name:   { title: [{ text: { content: title.trim().slice(0, 200) } }] },
    Status: { status: { name: 'New' } },
  };
  // Always file into the current sprint. Sprint is a multi_select like
  // "Sprint 73" — derive the option name from the current sprint number.
  try {
    const cur = await getCurrentSprint();
    const n = cur && cur.current && cur.current.number;
    if (n != null) props['Sprint'] = { multi_select: [{ name: `Sprint ${n}` }] };
  } catch (_) {}
  const pri = BUG_PRIORITY_MAP[priority];
  if (pri) props['Priority'] = { select: { name: pri } };
  // Always assign the PM (INCIDENT_SELF_NOTION_ID) in the "Assign" field.
  const self = SELF_NOTION_ID();
  if (self && /^[0-9a-fA-F-]{32,36}$/.test(self)) props['Assign'] = { people: [{ id: self }] };

  // Everything the board has no column for → page body, so nothing is lost.
  const children = [];
  if (opts.submittedBy) children.push(paraBlock(`👤 Created by ${opts.submittedBy}`));
  let whyText = why?.trim() || '';
  if (otherAreaText) whyText += `\n\nOther area: ${otherAreaText}`;
  if (whyText)                 children.push(paraBlock(`Why: ${whyText}`));
  if (metric)                  children.push(paraBlock(`Metric: ${metric.trim()}`));
  if (criticalReason && priority === 'Critical') children.push(paraBlock(`Critical reason: ${criticalReason.trim()}`));
  if (dueDate)                 children.push(paraBlock(`Due date: ${dueDate}`));
  if (areas?.length)           children.push(paraBlock(`Flow: ${areas.join(', ')}`));
  if (mockupLink)              children.push(paraBlock(`Mockup: ${mockupLink.trim()}`));
  if (mockupFileNames?.length) children.push(paraBlock(`Attached files: ${mockupFileNames.join(', ')}`));
  descriptionBlocks(description).forEach(b => children.push(b));

  const payload = { parent: { type: 'data_source_id', data_source_id: TASK_BOARD_DS_ID() }, properties: props };
  if (children.length) payload.children = children.slice(0, 100);
  const data = await napi('/pages', { method: 'POST', body: JSON.stringify(payload), headers: { 'Notion-Version': BUG_BOARD_VERSION } });

  // DM the creator a link to their new task so they can follow its status.
  // Best-effort and awaited (serverless freezes after the response) — a Slack
  // failure must never fail the task creation.
  if (opts.creatorSlackId) {
    try { await require('./slack').dmTaskCreated(opts.creatorSlackId, data.url, title.trim()); } catch (_) {}
  }
...
// Dev bug tasks live in the "Issues" data source, which is a "data source"
// model database — pages must be created against its data_source_id with the
// 2025-09-03 API (a database_id parent under the old API is rejected).
//
// Note: "Delivery Sprint Board" (11534c66…) is only a LINKED VIEW; the data
// actually lives in the "Issues" database (9a83cac4…) under the "Product
// Development" page. So we resolve from — and the integration must be shared
// with — the real source DB, not the linked board.
const BUG_BOARD_DB_ID      = () => process.env.BUG_BOARD_DB_ID || '9a83cac4205346489ef652a6d45d65a1'; // "Issues" (source)
const BUG_BOARD_DS_ID      = () => process.env.BUG_BOARD_DS_ID || '6a193143-2c58-4fb4-a17f-0105d466ab38'; // "Issues" data source
...
// Resolve the "Issues" data source id. Tries a live retrieve of the source
// database (survives id changes); falls back to the known data source id.
async function boardDataSourceId() {
  try {
    ...
    if (ds && ds.id) return ds.id;
  } catch (_) {}
  return BUG_BOARD_DS_ID();
}
const BUG_PRIORITY_MAP     = { Critical: '4. Critical', High: '3. High', Medium: '2. Medium', Low: '1. Low' };
const BUG_TAG_OPTIONS      = ['iOS', 'Android'];

// Fixed co-assignee always added to the auto-created task (the PM tracking all
// incidents). Notion user id; override with INCIDENT_SELF_NOTION_ID.
const SELF_NOTION_ID = () => process.env.INCIDENT_SELF_NOTION_ID || '299d872b-594c-8131-bef4-0002816b3e50'; // Emil Mubarakov
...
// Upload a single data: URL screenshot to Notion; returns a file_upload id (or
// null on any failure). Two steps: create the file upload, then send the bytes
// as multipart/form-data to its send URL.
async function uploadNotionImage(dataUrl) {
  const m = /^data:([^;]+);base64,(.+)$/.exec(dataUrl || '');
  if (!m) return null;
  const mime = m[1], bytes = Buffer.from(m[2], 'base64');
  const ext = (mime.split('/')[1] || 'png').replace('jpeg', 'jpg');
  const filename = `screenshot.${ext}`;
  const created = await napi('/file_uploads', { method: 'POST', body: JSON.stringify({ filename, content_type: mime }) });
  const id = created && created.id;
  const sendUrl = created && (created.upload_url || `${NOTION_API}/file_uploads/${id}/send`);
  if (!id || !sendUrl) return null;
  const tok = await loadToken();
  const fd = new FormData();
  fd.append('file', new Blob([bytes], { type: mime }), filename);
  const resp = await fetch(sendUrl, { method: 'POST', headers: { Authorization: `Bearer ${tok.access_token}`, 'Notion-Version': NOTION_VERSION }, body: fd });
  if (!resp.ok) return null;
  return id;
}

async function createBugTask(inc, opts = {}) {
  ...
  // Assignee: the picked dev (or the area map) PLUS a fixed co-assignee (the PM),
  // always — EXCEPT when "Nobody" (assigneeId 'none') is chosen, which assigns
  // no one at all. Guard id shapes; dedupe.
  const isUserId = v => typeof v === 'string' && /^[0-9a-fA-F-]{32,36}$/.test(v);
  let people = new Set();
  if (opts.assigneeId !== 'none') {
    people = new Set(isUserId(opts.assigneeId) ? [opts.assigneeId] : notionAssigneesFor(inc.areas));
    if (isUserId(SELF_NOTION_ID())) people.add(SELF_NOTION_ID());
  }
  if (people.size) props['Assignee'] = { people: [...people].map(id => ({ id })) };

  // The Notion "Reporter" property is a system created_by field (always the
  // API bot), so we can't set the human there. Record who reported it in the
  // body instead.
  const children = [];
  if (opts.reporterName) children.push(paraBlock(`👤 Reported by ${opts.reporterName}`));
  children.push(paraBlock('🐛 Auto-created from a Product Hub incident report'));
  ...
  // Attach the incident's screenshots as image blocks (best-effort — a failed
  // upload must not fail the task). Each is uploaded then appended to the page.
  const imgs = Array.isArray(inc.images) ? inc.images.slice(0, 3) : [];
  if (imgs.length) {
    const blocks = [];
    for (const im of imgs) {
      try { const fid = await uploadNotionImage(im); if (fid) blocks.push({ object: 'block', type: 'image', image: { type: 'file_upload', file_upload: { id: fid } } }); } catch (_) {}
    }
    if (blocks.length) {
      try { await napi(`/blocks/${data.id}/children`, { method: 'PATCH', body: JSON.stringify({ children: blocks }), headers: { 'Notion-Version': BUG_BOARD_VERSION } }); } catch (_) {}
    }
  }
  return { ok: true, url: data.url, id: data.id };
}
...
module.exports = {
  createTask, listTasks, getCurrentSprint, createFeedback, createBugTask, uploadNotionImage,
