...
async function createBugTask(inc, opts = {}) {
  ...
  // always — independent of who reported it. Guard id shapes; dedupe.
  const isUserId = v => typeof v === 'string' && /^[0-9a-fA-F-]{32,36}$/.test(v);
  const people = new Set(isUserId(opts.assigneeId) ? [opts.assigneeId] : notionAssigneesFor(inc.areas));
