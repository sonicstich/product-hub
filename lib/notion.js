...
// One-time helper: create the feedback database in Notion using the app's own
// OAuth token, under the same parent page as the Tasks DB (falls back to any
// page the integration can access). Because the app's integration creates it,
// the app automatically has write access — no manual "add connection" needed.
async function setupFeedbackDb() {
  const db = await napi(`/databases/${NOTION_DB_ID}`, { method: 'GET' });
  let parentPageId = db && db.parent && db.parent.page_id;
  if (!parentPageId) {
    const s = await napi('/search', { method: 'POST', body: JSON.stringify({ filter: { property: 'object', value: 'page' }, page_size: 1 }) });
    parentPageId = s && s.results && s.results[0] && s.results[0].id;
  }
  if (!parentPageId) throw new HttpError(400, 'No accessible parent page — share a page with the Product Hub integration first.');
  const created = await napi('/databases', {
    method: 'POST',
    body: JSON.stringify({
      parent: { type: 'page_id', page_id: parentPageId },
      title: [{ type: 'text', text: { content: 'Product Hub — User Feedback' } }],
      properties: {
        'Title':          { title: {} },
        'Description':     { rich_text: {} },
        'Users affected':  { number: {} },
        'Importance':      { select: { options: [
          { name: 'Low', color: 'gray' }, { name: 'Medium', color: 'yellow' }, { name: 'High', color: 'orange' },
        ] } },
        'User ID':         { rich_text: {} },
      },
    }),
  });
  return { id: created.id, url: created.url, parentPageId };
}
...
module.exports = {
  createTask, listTasks, getCurrentSprint, createFeedback, setupFeedbackDb,
