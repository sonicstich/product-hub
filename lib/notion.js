...
// The hub uses this as the anchor and derives everything else from it.
async function getCurrentSprint() {
  if (!token()) throw new HttpError(500, 'NOTION_TOKEN not configured');

  const resp = await fetch(`${NOTION_API}/databases/${SPRINTS_DB_ID}/query`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token()}`,
      'Notion-Version': NOTION_VERSION,
...
module.exports = { createTask, listTasks, getCurrentSprint, HttpError };
