...
// Slack "Sign in with Slack" (OpenID Connect) endpoints.
...
// Server-side gate for API handlers. When auth is configured, requests
// without a valid session get a 401 and this returns true (caller should
// `return`). When auth is off, always returns false (open).
function denyUnauth(req, res) {
...
function authorizeUrl(redirectUri, state) {
  ...
  return `${SLACK_AUTHORIZE}?${p.toString()}`;
}
...
async function exchangeCode(code, redirectUri) {
  ...
  catch (_) { throw new Error(`Slack token endpoint returned non-JSON (${resp.status})`); }
...
// ── Email/password accounts (limited "reporter" role) ──────────────────
// Provisioned in EMAIL_USERS — a JSON array of {email, name, salt, hash} where
// hash = scrypt(password, salt, 32) as hex. These sign in with email+password
// and get a session with role:'reporter' (can only report incidents). There is
// no self-signup — only the listed emails can log in.
function emailUsers() {
  try { const a = JSON.parse(process.env.EMAIL_USERS || '[]'); return Array.isArray(a) ? a : []; }
  catch (_) { return []; }
}
function verifyEmailUser(email, password) {
  const e = String(email || '').trim().toLowerCase();
  if (!e || !password) return null;
  const u = emailUsers().find(x => String(x.email || '').trim().toLowerCase() === e);
  if (!u || !u.salt || !u.hash) return null;
  let ok = false;
  try {
    const h = crypto.scryptSync(String(password), u.salt, 32);
    const stored = Buffer.from(u.hash, 'hex');
    ok = h.length === stored.length && crypto.timingSafeEqual(h, stored);
  } catch (_) { ok = false; }
  return ok ? { email: u.email, name: u.name || u.email } : null;
}
// True (and sends 403) when the session is a limited reporter — call on API
// endpoints reporters must not reach. Assumes denyUnauth already ran.
function denyRestricted(req, res) {
  const s = getSession(req);
  if (s && s.role === 'reporter') { res.status(403).json({ error: 'Forbidden' }); return true; }
  return false;
}

module.exports = {
  authConfigured, authorizeUrl, exchangeCode, allowedTeamId,
  signSession, verifySession, getSession, denyUnauth, denyRestricted, verifyEmailUser,
  sessionSetCookie, sessionClearCookie, COOKIE,
};
